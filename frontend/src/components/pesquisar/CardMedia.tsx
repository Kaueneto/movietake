...
export function CardMedia({ item, mediaType, customPosterPath }: Props) {
  ...
  const posterPath = customPosterPath ?? item.poster_path
    ...
    >
      ...
      <div className="relative aspect-[2/3] overflow-hidden bg-[#16161c]">
        {poster ? (
          ...
          <div className="flex h-full w-full items-center justify-center text-[#5a5a72]" aria-hidden="true">
      ...
      <div className="flex flex-1 flex-col gap-1 p-2">
        <h3 className="line-clamp-2 text-xs font-semibold leading-snug text-[#f1f1f3]" title={title}>
          {title}
        </h3>
        <div className="flex items-center gap-2">
          {year && (
            <span className="flex items-center gap-0.5 text-[10px] text-[#5a5a72]">
              <Calendar size={10} aria-hidden="true" />{year}
            </span>
          )}
          {rating && (
            <span className="flex items-center gap-0.5 text-[10px] text-[#5a5a72]" aria-label={`Nota ${rating}`}>
              <Star size={10} className="text-[#6366f1]" aria-hidden="true" />{rating}
            </span>
          )}
        </div>
      </div>
